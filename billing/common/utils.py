...
def app_meta_to_http_headers(app_meta):
    """
    Returns a set of app metadata into a dict
    of HTTP headers that can be fed to either a Webob
    Request object or an httplib.HTTP(S)Connection object

    :param app_meta: Mapping of app metadata
    """
    headers = {}
    for k, v in app_meta.items():
        if v is not None:
            if k == 'properties':
                for pk, pv in v.items():
                    if pv is not None:
                        headers["x-app-meta-property-%s"
                                % pk.lower()] = unicode(pv)
            else:
                headers["x-app-meta-%s" % k.lower()] = unicode(v)
    return headers
...
def get_app_meta_from_headers(response):
    """
    Processes HTTP headers from a supplied response that
    match the x-app-meta and x-app-meta-property and
    returns a mapping of app metadata and properties
    ...
    for key, value in headers:
        key = str(key.lower())
        if key.startswith('x-app-meta-property-'):
            field_name = key[len('x-app-meta-property-'):].replace('-', '_')
            properties[field_name] = value or None
        elif key.startswith('x-app-meta-'):
            field_name = key[len('x-app-meta-'):].replace('-', '_')
            result[field_name] = value or None
    result['properties'] = properties
    if 'size' in result:
        try:
